...
func _run_test() -> void:
	...
	if menu.catalog == null or menu.catalog.entries.size() != 6:
		failures.append("The level catalog should expose all 6 authored puzzles.")
	if menu.level_buttons.size() != 6:
		failures.append("The level-selection screen should create 6 launch buttons.")
	else:
	...
	if loaded_level == null or loaded_level.name != "DigBuildPuzzle":
		...
		if returned_menu == null:
			...
			if returned_menu.level_buttons.size() != 6:
